...
export default async function DiaporamaSlotPage({ params, searchParams }: Props) {
  ...
  const messages = await syliusGetMessages(slot);
  if (messages && messages.length > 0) {
    // si on a des messages, on les affiche à la place du diaporama produit
    console.log(`Messages found for slot "${slot}", redirecting to messages display.`);
